...
class ValidateByPortState(ValidateBase):
    def __init__(self, case, CPLD_CMD=None):
        ...
        self.portStateInitalPatterns = ['Standard port read: [a-f|0-9]+', 'Masked port read: [a-f|0-9]+']
        self.portStatePressPatterns = ["Port state: [a-f|0-9]+"]

    def validate(self):
        self.case.test(expectedPatterns=self.portStateInitalPatterns)
        self.case.test(inputStr="PRESS_BUTTON\r\n", expectedPatterns=self.portStatePressPatterns, spawnInput=self.case.assistSpawn)
...
class ValidateByFTM(ValidateBase):
    def __init__(self, case, CPLD_CMD=None):
        ...
        self.feqPatterns = [r'FREQ:(\d{1,2})',r'RATIO:[4-5]\d']
        self.feqTimeout = 60
    ...
    def validate(self):
        ...
        time.sleep(1)
        self.case.test(inputStr="FTM_0_ACT 0 EX_FTM_CAPTURE\r\n", expectedPatterns=self.feqPatterns, spawnInput=self.case.assistSpawn, spawnOutput=self.case.assistSpawn, timeout=self.feqTimeout)
...
class Kinetis(driver_examples):
    def __init__(self):
        ...
        self.header_patterns = ["GPIO\s*Driver\s*example"]
...
class hvpkv58f(Kinetis):
    def __init__(self):
        ...
        self.feqPatterns = [r'FREQ:[1-9]\d+',r'RATIO:\d+']
